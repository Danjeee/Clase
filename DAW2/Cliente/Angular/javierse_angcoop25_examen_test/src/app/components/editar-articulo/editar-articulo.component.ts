...
/**
  ...
  imagenPreview: SafeUrl | string =
    'http://localhost/pruebaServidor/imgs/articulos/'; // Imagen por defecto
  ...
  ngOnInit(): void {
    ...
    this.artService.getArticuloById(sessionStorage.getItem("art") as number |null).subscribe({
      next: (data) => {this.articulo = data[0]
        this.ventaForm.setValue({nombre: data[0].nombre, descripcion: data[0].descripcion, precio: data[0].precio, cat: data[0].categoria, foto: data[0].imagen})
      }
    })

    setTimeout(() => {
      const img = document.getElementById("imagen") as HTMLImageElement
      img.src = this.imagenPreview + this.articulo.imagen
    }, 200);
  ...
  subir(): void {
    ...
    this.ventasService.registrarVenta(formData).subscribe({
      next: (data) => {console.log(data)
        this.alertService.success("Exito", "Articulo editado correctamente")
      },
