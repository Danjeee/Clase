...
// (TS) = Todos los socios;
// (AA) = Todos los articulos (Excepto los vendidos);
// (TV) = Todas las ventas

if (
  $opc == "SR" || $opc == "SC" || $opc == "AC" || $opc == "AV" || $opc == "AS" || $opc == "TC" || $opc == "SVS" || $opc == "TS" || $opc == "AA" || $opc == "TV"
) {
  switch ($opc) {
    case "SR": // Socio Registrado
      $sql = "select * from socios where email = '" . $ema . "' and password = '" . $pas . "' and rol != 'I'";
      break;
    ...
    case "SVS": // Seleccionar los articulos que compra un usuario
      ...
      break;
    case "TS": // Seleccionar todos los socios
      $sql = "select * from socios where rol != 'A'";
      break;
    case "AA": // Seleccionar todos los socios
      $sql = "select * from articulos where estado != 'V'";
      break;
    case "TV": // Seleccionar todos los socios
  ...
  switch ($opc) {
    ...
    case "RV": // Registro Venta -----------------------------------------------------------------
      ...
      break;
    case "DS": // Desactivar socio -----------------------------------------------------------------
      $sql = "update socios set rol = 'I' where id = " . $ids;
      break;
    case "ES": // Activar socio -----------------------------------------------------------------
      $sql = "update socios set rol = 'U' where id = " . $ids;
      break;
    case "DA": // Desactivar articulo -----------------------------------------------------------------
      $sql = "update articulos set estado = 'I' where id = " . $ida;
      break;
    case "EA": // Activar articulo -----------------------------------------------------------------
      $sql = "update articulos set estado = 'D' where id = " . $ida;
      break;
  }
