...
export class NavbarComponent implements OnInit {
   ...
   */
  constructor(private alert: AlertService, private sessionService: SessionService, private router: Router) { }
  ...
  ngOnInit(): void {
    ...
    var index = "http://localhost:4200".length
    this.ruta = this.ruta.substring(index, this.ruta.length)
    ...
    if (this.ruta.startsWith("/admin")) {
      if (this.usuario == null) {
        this.alert.confirm("Error", "no puedes acceder aqui").then(() => { this.router.navigate(["/login"]) })
      }
      if (this.usuario.rol != 'A') {
        this.alert.confirm("Error", "no puedes acceder aqui").then(() => {
          if (!this.usuario) {
    ...
    if (!this.ruta.startsWith("/admin")) {
      try {
        if (this.usuario == null) {
          if (this.ruta != "/login" && this.ruta != "/registrosocio") {

            this.alert.confirm("Error", "No puedes acceder aqui").then(() => { this.router.navigate(["/login"]) })

          }
        } else
          if (this.usuario.rol != 'U') {
