...
-- Servicios
DROP TABLE IF EXISTS `clientesservicios`;
...
CREATE TABLE `servicios` (
  ...
  PRIMARY KEY (`id`));
  INSERT INTO `servicios`(nombre_servicio, descripcion) VALUES("Proteccion monetaria", "Seguro contra robos y fallos minimos al invertir"),("Inversión automatica","Ponemos a nuestros agentes inversores a utilizar parte de su dinero en inversiones con rentabilidad del 5% o superior"),("Crypto banco","Acceso a funciones relacionadas con criptomonedas y web3");
...
CREATE TABLE `clientesservicios` (
  ...
  PRIMARY KEY (`id`),
  INDEX `FK_user_idx` (`idcliente` ASC) VISIBLE,
  INDEX `FK_servicio_idx` (`idservicio` ASC) VISIBLE,
  CONSTRAINT `FK_user`
  ...
  CONSTRAINT `FK_servicio`
    ...
    REFERENCES `demospringboot`.`servicios` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE);
